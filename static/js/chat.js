    ...
    .then((data) => {
      if (data.status) {
        document.getElementById(
          "chat-with"
        ).innerText = `Chat con ${data.user.username}`;
        document.getElementById("chat-user-image").src =
          data.user.profile_picture;
        document.getElementById("chat-user-status").innerText =
          data.user.status || "En línea";
      ...
      if (data.messages.length === 0) {
        ...
        data.messages.forEach((msg) => {
          ...
          messageBubble.className =
            msg.sender === "me" ? "message-bubble me" : "message-bubble";
